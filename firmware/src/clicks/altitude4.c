...
void ALTITUDE4_readData(alt4_data_struct *alt4_data)
{
    ...
    uint16_t raw_pressure = app_deviceData.i2c.rxBuffBytes[1];
    raw_pressure = raw_pressure << 8;
    raw_pressure = raw_pressure | app_deviceData.i2c.rxBuffBytes[2];
    
    uint16_t raw_temperature = app_deviceData.i2c.rxBuffBytes[3];
    raw_temperature = raw_temperature << 8;
    raw_temperature = raw_temperature | app_deviceData.i2c.rxBuffBytes[4];
