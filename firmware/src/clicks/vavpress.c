...
uint16_t VAVPRESS_init()
{
    ...
    calID = app_deviceData.i2c.rxBuffBytes[25];
    calID = calID << 8;
    calID = calID | app_deviceData.i2c.rxBuffBytes[26];
...
void VAVPRESS_getSensorReadings(vav_data_struct *vav_data)
{
    ...
    raw_pressure = app_deviceData.i2c.rxBuffBytes[1];
    raw_pressure = raw_pressure << 8;
    raw_pressure = raw_pressure | app_deviceData.i2c.rxBuffBytes[0];
    ...
    raw_temperature = app_deviceData.i2c.rxBuffBytes[3];
