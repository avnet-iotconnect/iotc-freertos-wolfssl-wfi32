...
{
    APP_SENSORS_writeByte(addr, TEMPHUM14_CMD_RESET);
...
void TEMPHUM14_getTemperatureHumidity (uint8_t addr, temphum14_data_struct *temphum14_data)
{
    ...
    uint16_t raw_temperature = app_deviceData.i2c.rxBuffBytes[0];
    raw_temperature <<= 8;
    raw_temperature |= app_deviceData.i2c.rxBuffBytes[1];
    ...
    uint16_t raw_humidity = app_deviceData.i2c.rxBuffBytes[3];
    raw_humidity <<= 8;
    raw_humidity |= app_deviceData.i2c.rxBuffBytes[4];
...
uint32_t TEMPHUM14_getSerialNumber (uint8_t addr)
{
    ...
    ser_numb = app_deviceData.i2c.rxBuffBytes[0];
    ser_numb <<= 8;
    ser_numb |= app_deviceData.i2c.rxBuffBytes[1];
    ser_numb <<= 8;
    ser_numb |= app_deviceData.i2c.rxBuffBytes[2];
